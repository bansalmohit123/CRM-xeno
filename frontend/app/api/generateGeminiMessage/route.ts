...
//     const response = await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=" + process.env., {
//         method: "POST",
//         headers: {
...
export async function POST(req: Request) {
  try {
    ...
    // ✅ Error handling if API key is invalid or malformed response
    if (!data.candidates || !data.candidates[0]) {
      return NextResponse.json({
        success: false,
        error: data.error?.message || "No candidates returned by Gemini.",
      });
    }
